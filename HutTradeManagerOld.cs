...
public class HutTradeManagerOld
{
    ...
    public static async Task<bool> UpdateTradeAfterOffer(long tradeId, long offerId, long offererId, int bidCredits)
    {
        await using var conn = new NpgsqlConnection(Database.ConnectionString);
        await conn.OpenAsync();
    ...
    public static async Task ExecuteTrade(long tradeId, long offerId)
    {
        await using var conn = new NpgsqlConnection(Database.ConnectionString);
        await conn.OpenAsync();
        await using var transaction = await conn.BeginTransactionAsync();
        ...
        try
        {
            ...
            var offerInfo = await GetOffer(offerId);

            if (offerInfo.mCardDataList.Count >= 1)
            {
                foreach (var card in offerInfo.mCardDataList)
                {
                    await HutCardFactory.CreateOrUpdateCard(card, sellerId, DeckType.CARDHOUSE_DECK_UNASSIGNED);
                }
    ...
    public static async Task<ISOfferInfo> GetOffer(long offerId)
    {
        ...
        {
            return await HutHelper.ReadOffer(reader);
        }
        return new ISOfferInfo();
    }

    public static async Task CleanExpired()
    {
        await using var conn = new NpgsqlConnection(Database.ConnectionString);
        await conn.OpenAsync();
    ...
    public static async Task<long> GetWinningOfferId(long tradeId)
    {
        await using var conn = new NpgsqlConnection(Database.ConnectionString);
        await conn.OpenAsync();
